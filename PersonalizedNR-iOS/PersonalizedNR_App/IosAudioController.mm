...
//Added for path of noise classifier and user band gains ------------------
const char *noiseClassifierFile, *userBandGainsFile;
NSString   *str1, *str2;
//-----------------------------------------------------------------------
...
- (void) start {
    ...
    if (settings->saveData){ // For hybrid mode in unsupervised noise classifier and saving user band gains
        str1 = @"NoiseClassifier";
        NSArray* paths      = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);//path for file
        NSString* path3 = [paths objectAtIndex:0];
        NSString* Hybrid_dir = [path3 stringByAppendingString:[NSString stringWithFormat:@"/%@.dat",str1]];
        NSLog(@"%@" ,Hybrid_dir); // only for cheking file name format, NSLog is for display
        noiseClassifierFile = [Hybrid_dir UTF8String];
        
        str2 = @"UserBandGains";
        NSString* path4 = [paths objectAtIndex:0];
        NSString* userGains_dir = [path4 stringByAppendingString:[NSString stringWithFormat:@"/%@.dat",str2]];
        NSLog(@"%@" ,userGains_dir); // only for cheking file name format, NSLog is for display
        userBandGainsFile = [userGains_dir UTF8String];
    }
    
    vadNoiseRedCom = initVAD_NoiseReduction_Compression([self settings], noiseClassifierFile, userBandGainsFile);
    ...
    timer = [NSTimer scheduledTimerWithTimeInterval:0.5 // default GUI update rate is 0.5 seconds.
                                            repeats:YES
                                              block: ^(NSTimer* timer){
    ...
    vadTimer = [NSTimer scheduledTimerWithTimeInterval:0.0625 // VAD update rate is 62.5 ms.
                                               repeats:YES
                                                 block: ^(NSTimer* timer){
...
- (void) processStream {
    ...
    if (availableBytes > frameSize){
        ...
        // Perform VAD smoohting buffer here
        /*if(settings != NULL) {
            if (settings->VADSmoothBuffer == settings->VADBufferLength){ // buffer is full
                settings->VADSmoothBuffer = 0;
                settings->finalVADLabel = settings->classLabel; // change from "speech+noise" to either "Noise" or "Quiet" VAD class
            }
            else {
                settings->VADSmoothBuffer = settings->VADSmoothBuffer + 1; // buffer is filling; increasing counter by one
                settings->finalVADLabel  = 2; // still the VAD class is "speech+Noise".
            }
        }*/
